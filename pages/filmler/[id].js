  ...
  return (
    <Container fluid style={{ paddingLeft: 0, paddingRight: 0 }}>
      ...
      <div className="mainContainer">
        <Head>
          <title>
            {movie.title} ({year}) Filmi - Filmbul.org
          </title>
          <meta
            property="og:title"
            content={movie.title + " (" + year + ") " + "Filmi - Plumovi.com"}
            key="title"
          />
          <meta
            name="viewport"
        ...
        <div className="center">
          <div className="centerLeft">
            <div class="title">
              <h1>
                {movie.original_title != movie.title ? `${movie.original_title} : ${movie.title} Filmi` : `${movie.original_title} Filmi`}
              </h1>
            </div>
            {video.length > 0 && [
            ...
            <div className="topMovieCard">
              <div className="contentCard">
                ...
                <div className="contentCardRight">
                  ...
                  <div>
                    <span className="infoSpan">Vizyon Tarihi: </span>
                    <span>{movie.release_date}</span>
                  </div>
                  <div>
                    <span className="infoSpan">Ortalama Puan: </span>
                  ...
                  <div style={{ marginTop: "4px" }}>
                    <Button
                      variant="warning"
                      size="sm"
                      onClick={() => addLikeMovies(movie, year, id)}
                    >
                      {" "}
                      <FaHeart /> Sevdim
                    </Button>
                ...
                >
                  {
                    ((movie.poster_path =
                      ...
                      movie.poster_path),
                    (movie.href = `/filmler/${movie.title
                      .split(" ")
                      .join("-")}-${movie.id}`),
                    (
...
export async function getServerSideProps({ query }) {
  ...
  const resInfo = await fetch(
    `https://api.themoviedb.org/3/movie/${movieId}?api_key=94ec2f0211fe06a3b2bc9827439383d8&language=tr-TR`
  );
  ...
  await axios
    .get(
      `https://api.themoviedb.org/3/movie/${movieId}/similar?api_key=94ec2f0211fe06a3b2bc9827439383d8&language=tr-TR&page=1`
    )
    .then((res) => {
  ...
  await axios
    .get(
      `https://api.themoviedb.org/3/movie/${movieId}/recommendations?api_key=94ec2f0211fe06a3b2bc9827439383d8&language=tr-TR&page=1`
    )
    .then((res) => {
      dataRecom = res.data;
    });
